...
HK_CENTER = (22.3, 114.2) # 香港中心坐標，用於 AI 地圖路徑推算
...
# ⚠️ 已確保 Ensemble API 使用 ifs04，避免發生 Exit Code 1 報錯
...
try:
    ...
    if isinstance(res_ec, dict) and res_ec.get('error'): raise Exception(res_ec.get('reason'))
    res_ec_list = res_ec if isinstance(res_ec, list) else [res_ec]
    
    res_gfs = requests.get(url_gfs, timeout=15).json()
    if isinstance(res_gfs, dict) and res_gfs.get('error'): raise Exception(res_gfs.get('reason'))
    res_gfs_list = res_gfs if isinstance(res_gfs, list) else [res_gfs]

    res_dir = requests.get(url_dir, timeout=15).json()
    if isinstance(res_dir, dict) and res_dir.get('error'): raise Exception(res_dir.get('reason'))
...
for idx, t_str in enumerate(times):
    ...
    if center_dir is None: center_dir = 90
        
    # 🔥 修復 Bug 1：嚴格要求 idx >= 24 先計跌幅，徹底解決「大氣日夜潮汐」假陽性
    ...
    multiplier = 1.0
    if (center_dir >= 315) or (center_dir <= 45): multiplier = 0.8
    elif (center_dir >= 90) and (center_dir <= 180): multiplier = 1.05
    ...
    # 🔥 修復 Bug 2：防止除以零 (ZeroDivision) 導致 NaN 病毒
    ...
    results.append({
        "時間": display_time,
        "物理一號機率 (%)": prob_t1_phy, "物理八號機率 (%)": prob_t8_phy,
        "AI 一號機率 (%)": prob_t1_ai, "AI 八號機率 (%)": prob_t8_ai,
...
# ==========================================
downgrade_text = ""
# 🔥 修復 Bug 3：防彈機制，處理全空數據 (防止 idxmax 報錯)
...
if peak_prob >= 20.0:
    ...
    conclusion_html = f"""<div style="background: #2b2b00; padding: 15px; border-radius: 6px; margin: 15px 0; font-size: 15px; line-height: 1.6; border-left: 4px solid #ffd700; color: #fff;">⚠️ <b>AI 實時威脅判定：</b><br>系統目前偵測到八號風球信號，預計高峰期為<b>【{peak_time}】</b>，機率為 <b>{peak_prob}%</b> <span style="color:#aaaaaa;">(陣風分歧度：{peak_spread} km/h)</span>。目前威脅屬於中低度或處於分歧狀態，請密切留意。</div>"""
else:
...
def generate_ai_path(times_arr, winds_arr, dirs_dict):
    ...
    for i, t_str in enumerate(times_arr):
        ws = winds_arr[i]
        if ws < 40: continue # 只有陣風 >= 40km/h 才視為風暴逼近並畫出路徑
        
        wd = dirs_dict.get(t_str, 90)
        # 白貝羅定律：北半球背風而立，低壓中心在左前方 (約減 100 度)
        storm_bearing = (wd - 100) % 360
        # 根據陣風強度反向推算距離 (風越大，距離越近)
...
if len(ec_coords) >= 2:
    ai_paths_data["features"].append({"type": "Feature", "properties": { "model": "ECMWF (實時數據反向推算)", "color": "#a855f7" }, "geometry": { "type": "LineString", "coordinates": ec_coords }})

gfs_coords = generate_ai_path(times, gfs_mean_winds, dir_dict)
if len(gfs_coords) >= 2:
    ai_paths_data["features"].append({"type": "Feature", "properties": { "model": "GFS (實時數據反向推算)", "color": "#ef4444" }, "geometry": { "type": "LineString", "coordinates": gfs_coords }})
...
<body>
    <div class="container">
        <div class="header">
            <h1>🌀 香港潛在風暴雙軌早期預警系統 (終極 Data-Driven 版)</h1>
        ...
        <div class="intro-box">
            💡 <b>演算法終極升級：</b> 系統已修復「大氣日夜潮汐」造成的假陽性機率。地圖現已導入 <b>AI 白貝羅定律</b>，直接擷取實時風向與風速反向運算，生成 Data-Driven 預測路徑！(若陣風 <40km/h，地圖將自動保持清空)
        </div>

        <div>{chart_html}</div>
        
        <div id="typhoon-map"></div>
        <div style="font-size: 12px; color: #888; text-align: center; margin-top: 5px;">*地圖路徑由本系統 AI 基於 Open-Meteo 實時風向/風速動態推算。</div>
    ...
    <script>
        ...
        legend.onAdd = function () {{
            ...
            div.innerHTML = `
                ...
                <div class="legend-item"><div class="legend-color" style="background:#a855f7;"></div>ECMWF 數據反向推算</div>
                <div class="legend-item"><div class="legend-color" style="background:#ef4444;"></div>GFS 數據反向推算</div>
            `;
            return div;
        }};
        legend.addTo(map);
...
print("🎉 恭喜！包含 Data-Driven 地圖及修正時間蟲嘅終極版 index.html 已成功生成！")
