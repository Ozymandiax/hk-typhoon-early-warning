...
X_train = [
    # 0: 日常 / 午後氣壓下降 / 海陸風 (噪音過濾)
    [1010, 0.0, 15], [1008, 1.5, 25], [1006, 2.0, 30],
    [1008, -1.0, 20], [1005, 0.5, 22], [1007, 1.0, 18],
    
    # 1: 颱風胚胎靠近 / 外圍影響
    [1004, 3.0, 25], [1002, 3.5, 30], [1003, 3.2, 28],
    [1005, 2.8, 32], [1001, 4.0, 33],
    
    # 3: 強風圈觸及 (T3 級)
    [1000, 5.0, 35], [999, 6.0, 38], [1001, 4.5, 36],
    [1002, 4.0, 40], [998, 5.5, 42],
    
    # 8: 核心烈風 / 模型低估下的 T8 情況
    # 重點：加入「壓力沒掉很深，但風速已達模型高值」的樣本
    [1002, 5.0, 42], [998, 7.0, 48], [990, 10.0, 55],
    [1004, 4.5, 45], [1003, 5.5, 43], [1001, 6.0, 47],
    [1005, 3.8, 44], [999, 8.0, 50], [1000, 6.5, 46]
]
...
for idx, t_str in enumerate(times):
    ...
    # 軌道 1：改進後的物理門檻判定邏輯
    # ------------------------------------------
    # T1：氣壓 ≤ 1006 且 24h 降幅 ≥ 2.0（稍微放寬）
    ec_t1_phy = (ec_press <= 1006) & (ec_press_drop >= 2.0)
    gfs_t1_phy = (gfs_press <= 1006) & (gfs_press_drop >= 2.0)
   
    # T8：更貼近模型實際表現的門檻（重要改進）
    # 條件1：氣壓已有明顯下降 + 風速達模型中等偏高值
    # 條件2：單純風速已達較高值（捕捉模型低估下的烈風）
    ec_t8_phy = ((ec_press <= 1005) & (ec_winds >= 35)) | (ec_winds >= 42)
    gfs_t8_phy = ((gfs_press <= 1005) & (gfs_winds >= 35)) | (gfs_winds >= 42)
...
fig.add_trace(go.Scatter(x=df_res_filtered["時間"], y=df_res_filtered["物理一號機率 (%)"], 
                         name="🟡 改進物理 (一號風球)", line=dict(color='yellow', width=2, dash='dash')))
fig.add_trace(go.Scatter(x=df_res_filtered["時間"], y=df_res_filtered["AI 一號機率 (%)"], 
                         name="🌟 AI 決策樹 (一號風球)", line=dict(color='gold', width=3)))
fig.add_trace(go.Scatter(x=df_res_filtered["時間"], y=df_res_filtered["物理八號機率 (%)"], 
                         name="🔵 改進物理 (八號風球)", line=dict(color='deepskyblue', width=2, dash='dash')))
fig.add_trace(go.Scatter(x=df_res_filtered["時間"], y=df_res_filtered["AI 八號機率 (%)"], 
                         name="🔴 AI 決策樹 (八號風球)", line=dict(color='red', width=3)))

fig.update_layout(
    title="🌀 香港風暴信號預測：改進物理門檻 vs AI 決策樹",
    yaxis_title="發出機率 (%)",
    xaxis_title="預測時間",
    hovermode="x unified",
...
<head>
    ...
    <title>香港潛在風暴雙軌早期預警系統 (改進版)</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>🌀 香港潛在風暴雙軌早期預警系統 (改進版)</h1>
            <div class="update-time">最後自動更新（香港時間 HKT）: {update_time_str}</div>
        </div>
       
        <div class="intro-box">
            💡 <b>改進重點：</b><br>
            • <b>物理八號門檻</b>已放寬為「氣壓 ≤ 1005 + 風速 ≥ 35」或「風速 ≥ 42」，更符合全球模型對近岸風速的低估特性。<br>
            • AI 訓練資料已加入更多「中心較遠但仍有較強風力」的樣本。<br>
            • 虛線 = 改進後物理門檻；實線 = AI 決策樹。
        </div>
        <div>{chart_html}</div>
        ...
        <div class="table-container">
            ...
            {table_html}
            <div class="note">
                注意：此系統使用單一格點 + 模型風速，仍會低估實際掛八號的機率。建議同時參考專門的 TC Track Ensemble（如 FNV3）作為交叉驗證。
            </div>
        </div>
    </div>
</body>
